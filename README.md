...
## Настройка analog (регламент оператора)

Для стабильного распознавания analog в `Logger setup` используйте пошаговый сценарий:

1. Нажмите `Refresh snapshot` и дождитесь четкого кадра.
2. Шаг `ROI`: выделите только шкалу и стрелку (без лишнего фона).
3. Шаг `Center`: кликните точно в ось стрелки.
4. Шаг `Min point`: кликните направление стрелки на минимуме шкалы.
5. Шаг `Max point`: кликните направление стрелки на максимуме шкалы.
6. Введите `Scale values` (`min_value`, `max_value`) для прибора.
7. Нажмите `Save config`.
8. Нажмите `Test recognize` и затем `Quality x6`.

### Критерии приемки калибровки

- В блоке `Quality gate` должен быть статус `PASS`.
- Нет частых предупреждений `needle_not_found`.
- Значения не должны залипать в крайних точках (`0%/100%`) без реального достижения края шкалы.

### Если quality не пройден

- Сузьте ROI до самого циферблата.
- Переставьте `Center/Min/Max` точки.
- Уменьшите блики и дрожание камеры.
- Проверьте, что `Min` и `Max` не перепутаны.

Важно: если ROI изменен после сохранения, калибровка считается устаревшей. Нужно снова расставить точки и сохранить конфигурацию.
